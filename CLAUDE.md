...
- 开工须先获总司令明确指令，当前状态：✅ 子项目 1–1.9 全部完工 + 子项目 2.0「球队主页赛程」完工（2026-08-21）+ 积分榜前端实时化 + 球员履历末段兜底（2026-08-24：赛程/积分榜同源同实时，履历当前队兜底修复转会滞后；337 单测全绿；已 push）+ 子项目 2.1「旗面真彩重设计」完工（2026-08-28：349 单测全绿；已 push）+ 中超积分榜数据质量三件套（2026-08-31：ESPN 勘误层 + 同分相互战绩排序 + 延期场判据根治 + Actions 体检红灯；375 单测全绿；已 push）；待总司令批军衔 + 决定下一子项目方向
...
五大联赛（英超/西甲/意甲/德甲/法甲）足球数据查询网站，**Phase 0–6 MVP + 子项目 1–1.9（个人化 + 移动端兼容系列）均完工**（线上部署 https://davidgao1024.github.io/MatchLab/）：Vite 6 + Vue 3 + TS strict + Tailwind 4 脚手架、hash 路由 + Pinia、部署工作流、数据管线（fetch-espn-core / fetch-understat / fetch-espn-scores / build-team-map / fetch-dqd-players 五个零依赖脚本 + 每日定时工作流）均已入库，`public/data/` 含六联赛静态数据（球队 96+ / 球员 2611 / 队名映射 26 条 / 球员中英译名 31393 变体）；前端积分榜 + 赛程 + 首页 + 比赛详情弹窗 + 球员/球队详情（球员含**转会履历**）+ 排行榜 + 对比页 + 个人化（球队订阅 + 收藏夹 + iCal 导出 + 伤员端点 + 隐私模式 readOnly）齐上线（349 项单测全绿，typecheck/build 通过）；2026-08-18 合规收尾（国旗/队徽本地化 + xG 舍弃 + 免责声明 + 降频）+ 新赛季切换；2026-08-24 积分榜前端实时计算 + 球员履历当前队兜底；2026-08-28 球队旗面真彩重设计（主色实底 + 副色针纹 + 纯色边框，accent 点缀全收）。
