...
### 2026-09-23 — ESPN 废弃 scoreboard 日期区间语法：整月令牌替代（逐场等价已验证）

**背景**：查岗发现 Fetch Data 自 09-17 起连红 4 次（09-17/19/20/21），静态数据冻结在 09-14（部署工作流正常，09-10 解耦生效未被连坐）。失败步「Fetch ESPN scores + compute standings」；Actions 日志需鉴权取不到，改本地复现定位。

**结论**：
- **区间语法已死**：`dates=YYYYMMDD-YYYYMMDD` 一律 **HTTP 400**（响应体 `{"code":400,"message":"Failed to get events endpoint."}`）。非网络/UA/限流问题——任何宽度（当日/2 日/7 日/整月）与任何变体（带或不带 limit、`2025-07-01-2025-07-31`、逗号、`region=us`、`seasons/`参数）全 400；**单日 `dates=YYYYMMDD` 仍 200**，curl UA 与浏览器直连均正常。
- **整月令牌 `dates=YYYYMM` 可用** ✅：与旧区间产物**逐场 eventId 完全等价**（实测 `eng.1 2025-09` 30/30、`esp.1 2025-09` 39/39、`chn.1 2026-09` 11/11，比对基准为 09-14 区间尚可用时生成的静态月文件）。另测 `dates=YYYY`（赛季令牌）只返 100 条截断，不可用。
- **连带高危（比红灯本身更险）**：`resolveSeasonsInPlace` 赛季交替探测同样走区间 URL，400 被 `catch {}` **静默吞掉** → `league.season` 停在旧季 2025（本地复跑日志 `[season] eng.1 → 2025（候选 2026）` 实锤）。**只修取数不修探测，管线会把 2025-26 旧季数据覆写进 2026-27**。
- **前端同源失能**：`fetchLiveScores`（积分榜当月直播）与 `fetchScoresRange`（首页昨日战报）均用区间 URL → 线上两块同坏（战报带 `catch` 吞掉成空窗隐藏）。

**落地**（一 commit `e2144c3e`，已 push）：① `scripts/lib/espn-endpoints.js` 新增 `monthToken()`；赛季探测改月令牌并**失败改为出声明**（堵静默吞错，同类伤不再无声）；② `fetch-espn-scores.js` 月窗口改月令牌；③ 前端 `monthDateRange` → `monthToken`（`dates=YYYYMM`），`fetchScoresRange` 重写为「按覆盖月逐月取月令牌 + 本地按日过滤（两端各放宽 1 天吸收北京/UTC 跨日），月数据复用 fetchLiveScores 的 60s 缓存」。

**验证**：六联赛端到端跑通（赛季正确切 2026、**旧赛季文件零覆写**、08 月 20 场 + 09 月 30 场 = 50 场 = 5 轮与榜首 5 战 15 分吻合）；体检 exit 0（0-0 联网核验 140 场无异常）；403 单测全绿、typecheck/build 过；浏览器实测本地与线上（GH Pages）积分榜 2026-27 与昨日战报带均正常，线上六联赛 `dates=202609` 全 200。
...
**结论**：
- **ESPN transactions 复测**：`athletes/274197/transactions`（李刚仁）现只返回 1 条 2023-07-08 马洛卡→巴黎，缺「巴黎→马竞」；带 `?seasons=2026` 无效；联盟级 `leagues/esp.1/transactions` 仍空。确认为数据源本身滞后（非抓取 bug），无隐藏端点可拿更新数据。
- **德转复测**：仍 405 + `x-amzn-waf-action: captcha`（AWS WAF），脚本不可用。
- **当前球队字段准确**：ESPN core roster 李刚仁 teamId=1068 已显示马竞，仅转会履历滞后。

**落地**：① 积分榜改**前端实时算**（当月直播直连 + 历史月静态 JSON → `src/utils/standings.ts` computeStandings，中超扣分随迁）；② `buildCareer` 显式传当前队兜底——当前队 ≠ 最后转入队时原队收尾当前年 + 追加当前队至今（运行时兜底，ESPN 补录后自动切回）；③ 升班马+新援缺译名（懂球帝 08-17 停抓所致，五大赛覆盖降至 73–81%），总司令拍板「先接受现状」。
...
**背景**：08-13 总司令报线上订阅天津津门虎后订阅卡旗面仍是红色。ESPN 源数据对天津返回深红 `#C60000`（实际 2026 赛季主场紫色 + 白色细节），纠偏代码（TEAM_OVERRIDES 覆盖表，紫 `#5B2D8B` + 白）08-12 已在 main，但线上仍派发红色。排查连起三个问题。

**问题一 —— 37 秒撞车（红色数据怎么上的线）**：
- 08-12 09:21/09:26 UTC 纠偏两步提交（f392c29 初版紫+黄、d199cb0 黄改白）先后推送
- 每日抓取任务在纠偏推送**之前**已开跑（拿的旧代码），却于 09:27:16 UTC 才跑完提交——比纠偏推送晚 37 秒，红色数据盖上 main 并随之上线

**问题二 —— 部署断链（修好的数据为什么没部署）**：
- 08-13 手动补跑抓取，数据提交 81f8fc3 上 main，40 分钟后线上仍派发红
- 根因：抓取任务的提交身份是 Actions 运行时机器人令牌（GITHUB_TOKEN），GitHub 防死循环规则下该令牌产生的推送**不触发其他工作流**——`deploy.yml` 的 `on: push` 从未点燃
- 08-12 红色之所以能上线，是总司令手动推送合并提交顺带点燃了部署，纯属运气

**问题三 —— 缓存滞留（部署了为什么还看不见）**：
- `teams.json` 走 fetchJsonCached 的 24 小时 localStorage 缓存且无 forceFresh 覆盖；积分榜/排行榜/赛程早已"进页面取最新"，球队包是覆盖缺口
- 纠偏部署后老访客最长滞留 24 小时才见新色（本次总司令普通窗口即实例，无痕窗口立见紫佐证线上已修）

**修复**（提交 2f8dd89 + 8c8dc25）：
- `deploy.yml` 增加 `workflow_run` 触发器，监听 Fetch Data 完成后接力部署，加"抓取成功才部署"条件闸；push 与手动触发路径不受影响；当日推送实战绿勾验证
- `teams.ensure` 增加 forceFresh 支持（照 leaders 模式：跳过内存缓存 + 绕过本地缓存重取写回）；订阅卡与收藏夹页两个"队色队徽可见"入口传 forceFresh:true；其余页面保持 24h 缓存零开销；新增 4 项测试，全套 256 项绿
- 紧急纠偏操作路径确认：Actions 页手动触发 "Fetch Data" 立即重抓，部署自动接力（本次手动部署属断链期间的临时手段，今后不再需要）

**遗留风险**：
- 页面自身缓存（index.html 启发式缓存）不受任何方案控制，极端情况老访客需强刷一次
- Fetch Data 红灯时接力部署不会发生（条件闸有意为之），红灯必查——空榜防线同理
...
#### 决策：ESPN 日期区间语法废弃的适配——整月令牌 `dates=YYYYMM` + 本地按日过滤
**日期**：2026-09-23
**背景**：ESPN 单方面废弃 scoreboard 区间查询（2026-09 起一律 400 "Failed to get events endpoint."），管线取数、赛季探测、前端积分榜当月、首页昨日战报四处同源失能，静态数据冻结 9 天
**选项**：A) 逐日单日请求（每月 30 次/联赛）B) 整月令牌 `dates=YYYYMM` C) 赛季令牌 `dates=YYYY` D) 改走 core API events（仅服务端可用）
**选择**：B（前端区间语义改由本地按日过滤实现）
**理由**：A 请求量 ×30（12 月 ×6 联赛 ≈ 2160 次/趟）徒增限流风险与耗时；C 实测只返 100 条截断；D 浏览器 CORS 未验证且前端必须直连、解决不了线上。B 单月一请求、**实测与旧区间结果逐场 eventId 等价**，是最小改动路径
**后续影响**：**数据源约定新增一条——site.api 取数一律用单日或整月令牌，禁用区间语法**（新脚本/新端点沿用）；前端 `fetchScoresRange` 由服务端过滤变为「覆盖月逐月取数 + 本地按日过滤（±1 天时区松弛）」，跨月区间变 2 请求（原 1 请求）、月数据复用 `fetchLiveScores` 60s 缓存；赛季探测的 `catch {}` 改为失败出声明（本次 400 曾被静默吞掉、season 险停旧季）；同类外部变更的探查路径固化为「本地复跑脚本复现 → 探测端点语法变体 → 与既有静态产物逐场比对」
...
**日期**：2026-08-13
**背景**：机器人令牌（GITHUB_TOKEN）的数据提交不触发 push 事件，每日数据更新后 deploy.yml 不点燃，线上数据滞留等人工推送顺带点燃（08-13 天津纠偏数据滞留 main 40 分钟，靠手动部署收场）
**选项**：A) deploy.yml 加 workflow_run 触发器监听 Fetch Data 完成接力 B) 改用 PAT（个人访问令牌）推送，让推送事件正常触发 C) 部署工序并进 fetch-data.yml 单文件
**选择**：A
**理由**：workflow_run 是官方"机器人提交不触发下游"的标准解；B 要在 secrets 放写入令牌，多轮换管理负担；C 让构建部署逻辑两处重复，且代码推送仍需 deploy.yml。A 无额外密钥负担，加"抓取成功才部署"条件闸避免红灯后空跑
**后续影响**：每日 Fetch Data 跑完多一趟约 3 分钟部署（数据无变化的日子也算，月增约 90 分钟，额度内）；workflow_run 只监听默认分支——手动触发 Fetch Data 须选 main；08-14 定时任务验证接力路径

#### 决策：球队包进页面取最新——数据纠错当天可见（方案甲）
**日期**：2026-08-13
**背景**：teams.json 走 24h localStorage 缓存且无 forceFresh 覆盖，天津颜色纠偏部署后老访客最长滞留 24 小时；积分榜/排行榜/赛程已有"进页面 forceFresh"模式，球队包是缺口
**选项**：A) 球队包照搬既有 forceFresh 模式（订阅卡 + 收藏夹入口）B) 缓存键挂数据版本号，每日更新全站缓存集体作废 C) 球队包 TTL 从 24h 缩到 1h
**选择**：A
**理由**：A 贴合现成机制（5 分钟节流防泛滥），只在"队色队徽可见"两入口各多一个小请求，纠偏即时可见；B 改动面大且版本号自身要先取到（先有鸡先有蛋）；C 只是把等待从 24h 缩到 1h，治标不治本
**后续影响**：teams.ensure(league, {forceFresh}) 跳内存缓存 + 绕本地缓存重取写回；useLeague（各联赛页球队包）保持 24h 缓存零开销；极紧急纠错仍保留 bump CACHE_PREFIX 版本号全站缓存作废的兜底手段；页面自身启发式缓存不受控，极端情况老访客需强刷一次
