...
- [x] **Step 1: 收藏夹页手测（375px 视口，DevTools 设备模拟）**（2026-08-12 已测，10 项全过）
...
- [x] **Step 2: 桌面复查（1024 / 1440）**（2026-08-12 已测，无破相；球队详情页导出钮旧文案旧样式确认）

收藏夹页样式与全站统一、无破相；顺带确认球队详情页的导出按钮仍是旧文案旧样式（回归 Task 1 默认分支未变）。

- [x] **Step 3: 其余四页 375px 复查（含英文模式）**（2026-08-12 已测，中英双语零溢出零残留，无真问题）
...
- [x] **Step 4: 记录结论**（见文末「手测结论」一节）

- 全部通过 → 向总司令汇报完工，附复查结论
- 发现真问题 → 记入汇报，按 spec 风险表另起 followup（一行可修的除外，修了要单独说明）
- 未获浏览器许可 → 汇报「代码完工待回归」，清单原样移交

- [x] **Step 5: 恢复窗口**（2026-08-12 已恢复 ≥1024px）

浏览器窗口 ≥1024px，关闭 DevTools 移动模拟。

---

### 手测结论（2026-08-12 补测，总司令亲批浏览器许可）

**Step 1 收藏夹页 10 项全过**：暗色卡片统一 / 球队行队徽+名字+徽章+日历+删除一行无横滑 / 无编号条目降级 / 名字跳详情 / 日历钮 .ics 落盘（阿森纳 38 场）+ toast（观察器实证存续 3 秒）/ 删除即删 + toast / 球员行黄星 + 跳详情 / 页签计数与中性下划线 / 空态引导 + CTA 跳积分榜 / 英文模式零中文残留且无横滑（昨日横滑修复实证有效）。队徽 CDN 本机偶发超时，两段兜底（普通版→首字圆牌）按设计生效。

**Step 2 桌面 1024/1440 无破相**；球队详情页导出钮旧文案「导出赛程到日历」+ 旧样式确认（任务 1 默认分支未变）。

**Step 3 四页复查无真问题**：赛程页月份条容器内横滑、完赛/未开赛两态齐（进行中态当下无比赛可遇，逻辑为存量代码）、日期组头正常；球队详情返回钮/三按钮/战绩块/阵容分组正常；球员详情基础信息/xG 块/两趋势图/折叠分类正常；首页订阅引导/上轮战报带/联赛板块正常。中英双语四页 scrollWidth 均 = 375，英文零中文残留。

**观察级记录（不修，非真问题）**：
1. 空态 CTA「去积分榜找球队」与「订阅主队」钮为 MVP 存量共享蓝色样式，与暗色风格不统一——EmptyState/SubscribeButton 不在本批清单，红线不碰。
2. 英文模式 375px 长队名截断较狠（"Borus…"），徽章按钮完整，属预期美观级。
