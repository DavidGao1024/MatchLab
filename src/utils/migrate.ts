...
function isSubscription(x: any): x is Subscription {
  return x && typeof x.league === 'string' && typeof x.teamId === 'number' && typeof x.teamName === 'string' && typeof x.addedAt === 'string'
}

function isFavorite(x: any): x is Favorite {
  return x && typeof x.league === 'string' && (typeof x.name === 'string' || typeof x.teamName === 'string') && typeof x.addedAt === 'string'
}

function normalizeFavorite(x: any): Favorite {
  const out: Favorite = { league: x.league, name: x.name ?? x.teamName, addedAt: typeof x.addedAt === 'string' ? x.addedAt : new Date().toISOString() }
